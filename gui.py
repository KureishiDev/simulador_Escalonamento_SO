...
THEMES = {
    "dark": {
        "header_footer": "#23272A", "background": "#2C2F33", "canvas_bg": "#36393F",
        "text": "#FFFFFF", "text_axes": "#99AAB5", "button": "#40444B",
        "button_active": "#5C6067", "title_bar_mode": 1
    },
    "light": {
        "header_footer": "#E3E5E8", "background": "#FFFFFF", "canvas_bg": "#F8F9F9",
        "text": "#060607", "text_axes": "#5C6773", "button": "#E3E5E8",
        "button_active": "#B9BBBE", "title_bar_mode": 0
    }
}
...
#classe que cria as caixinhas de dicas ao sobrepor o ponteiro do mouse por cima
class Tooltip:
...
class AppGUI:
    def __init__(self, master):
        ...
        self.main_content_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=CANVAS_PADDING_Y, padx=10)
   ...
   #aplica as cores do tema de janela claro|escuro
    def _apply_theme(self):
        ...
        self.style.configure('TButton', font=('Segoe UI', 10, 'normal'), padding=(10, 5), background=colors["button"], foreground=colors["text"], borderwidth=0, focusthickness=0)
        ...
        self.header_label.configure(background=colors["header_footer"], foreground=colors["text"])
        self.footer_label.configure(background=colors["header_footer"], foreground=colors["text"])

        self.style.configure("Treeview", background=colors["canvas_bg"], foreground=colors["text"], fieldbackground=colors["canvas_bg"], borderwidth=0)
        self.style.configure("Treeview.Heading", background=colors["button"], foreground=colors["text"], padding=5, font=('Segoe UI', 10, 'bold'))
        self.style.map("Treeview.Heading", background=[('active', colors["button_active"])])

        if hasattr(self, 'logo_label') and self.logo_label: self.logo_label.configure(bg=colors["header_footer"])
    ...
    def _setup_header(self): #cria e posiciona os elementos do cabecalho
        ...
        try:
            img_light = Image.open(resource_path("github-mark-white.png")).resize((32, 32), Image.Resampling.LANCZOS)
            img_dark = Image.open(resource_path("github-mark.png")).resize((32, 32), Image.Resampling.LANCZOS)
        ...
        try:
            img_book = Image.open(resource_path("todos.png")).resize((32, 32), Image.Resampling.LANCZOS)
            photo_book = ImageTk.PhotoImage(img_book)
            self.book_icon_label = tk.Label(left_icons_frame, image=photo_book, cursor="hand2")
        ...
        self.header_label = ttk.Label(center_title_frame, text=header_text, font=("Segoe UI", 16, "bold"), anchor="center")
        self.header_label.pack(expand=True, fill=tk.BOTH)

    def _setup_widgets(self): #cria e posiciona os botoes e o canva principal
        button_container = ttk.Frame(self.control_frame)
        button_container.pack()
        self.btn_load = ttk.Button(button_container, text="Carregar Arquivo...", command=self._load_file_clicked)
        self.btn_load.pack(side=tk.LEFT, padx=5)
        Tooltip(self.btn_load, "Carrega um novo arquivo de simulação (.txt)")

        self.btn_show_data = ttk.Button(button_container, text="Ver Dados do Arquivo", command=self._show_data_dialog, state="disabled")
        self.btn_show_data.pack(side=tk.LEFT, padx=5)
        Tooltip(self.btn_show_data, "Mostra os dados de entrada do arquivo de simulação carregado")

        self.btn_next_step = ttk.Button(button_container, text="Próximo Passo", command=self._next_step_clicked, state="disabled")
        self.btn_next_step.pack(side=tk.LEFT, padx=5)
        Tooltip(self.btn_next_step, "Avança a simulação em um único tick de tempo")
        self.btn_run_all = ttk.Button(button_container, text="Executar Tudo", command=self._run_all_clicked, state="disabled")
        self.btn_run_all.pack(side=tk.LEFT, padx=5)
        Tooltip(self.btn_run_all, "Inicia ou para a execução automática da simulação")
        self.btn_save = ttk.Button(button_container, text="Salvar Imagem", command=self._save_canvas_as_image, state="disabled")
        self.btn_save.pack(side=tk.LEFT, padx=5)
        Tooltip(self.btn_save, "Salva o gráfico de Gantt atual como uma imagem PNG")
        
        self.lbl_tick = ttk.Label(self.control_frame, text="Tick: --", font=("Segoe UI", 12))
        self.lbl_tick.pack(side=tk.RIGHT, padx=20)
        self.lbl_algorithm = ttk.Label(self.control_frame, text="Algoritmo: --", font=("Segoe UI", 12))
        self.lbl_algorithm.pack(side=tk.RIGHT, padx=20)
        self.canvas = tk.Canvas(self.canvas_frame, scrollregion=(0,0,5000,2000), highlightthickness=0)
    ...
    def _setup_footer(self): #posiciona o rodape
        ...
        self.theme_button = ttk.Button(left_footer, text="Ativar Modo Noturno", command=self._toggle_theme)
        ...
        footer_text = "Simulador de Escalonamento v1.5 | 2025 Vinicius Wandembruck | Licença MIT"
        self.footer_label = ttk.Label(center_content, text=footer_text, font=("Segoe UI", 10))
        self.footer_label.pack(side=tk.LEFT, padx=10)
        
        self.btn_about = ttk.Button(right_footer, text="Sobre", command=self._show_about_dialog)
        self.btn_about.pack(expand=True, padx=10)
        Tooltip(self.btn_about, "Exibe informações sobre o projeto e referências")

    def _open_github_link(self, event=None): #abre este repositorio no github
        url = "https://github.com/KureishiDev"
        webbrowser.open_new_tab(url)

    def _open_book_link(self, event=None): #abre a referencia do livro do prof.maziero
        url = "https://wiki.inf.ufpr.br/maziero/lib/exe/fetch.php?media=socm:socm-06.pdf"
        webbrowser.open_new_tab(url)

    def _set_title_bar_theme(self, theme_mode): #altera a cor da barra de titulo
        try:
            import ctypes
    ...
    def _toggle_theme(self): #chamada botao tema claro|escuro
        self.is_dark_theme.set(not self.is_dark_theme.get())
        if self.is_dark_theme.get(): self.theme_button.config(text="Ativar Modo Claro")
        else: self.theme_button.config(text="Ativar Modo Noturno")
        self._apply_theme()

    def _show_data_dialog(self): #mostra os dados do arquivo selecionado em uma tabela
        ...
        for task in self.simulator.original_task_list:
            tree.insert("", tk.END, values=(task.task_id, task.color, task.arrival_time, task.duration, task.priority))
        ...
        ok_button = ttk.Button(button_frame, text="OK", command=data_window.destroy, style='TButton')
        ok_button.pack()
        
    def _show_about_dialog(self): #janela sobre pop-up
        ...
        about_text = ("Referência principal para a confecção dos algoritmos:\n"
        ...
        ttk.Label(content_frame, text=about_text, justify=tk.LEFT, font=("Segoe UI", 11)).pack(pady=10)
    ...
    def _load_file_clicked(self): #chamada pelo botao carregar, p selecionar o txt
        file_path = filedialog.askopenfilename(title="Selecione o arquivo de configuração", filetypes=[("Arquivos de Texto", "*.txt"), ("Todos os arquivos", "*.*")])
    ...
    def _reset_simulation_gui(self): #limpa o canvas e reseta botoes para o estado inicial
        self.is_running = False
        self.task_y_positions = {}
    ...
    def _draw_axes(self): #desenha os eixos x | tempo  e y | tarefas
        self.canvas.delete("axes")
        theme = "dark" if self.is_dark_theme.get() else "light"
        colors = THEMES[theme]
        axis_color = colors["text_axes"]
        
        self.canvas.create_line(X_OFFSET, Y_OFFSET - 20, 5000, Y_OFFSET - 20, arrow=tk.LAST, fill=axis_color, tags="axes")
        self.canvas.create_text(X_OFFSET + 40, Y_OFFSET - 35, text="Tempo (ticks)", fill=axis_color, font=("Segoe UI", 9), tags="axes")
        for i in range(0, 151):
            x = X_OFFSET + i * TICK_WIDTH
            self.canvas.create_line(x, Y_OFFSET - 25, x, Y_OFFSET - 15, fill=axis_color, tags="axes")
            if i % 5 == 0:
                self.canvas.create_text(x, Y_OFFSET - 35, text=str(i), font=("Segoe UI", 8), fill=axis_color, tags="axes")
        
        self.canvas.create_line(X_OFFSET, Y_OFFSET - 20, X_OFFSET, 2000, arrow=tk.LAST, fill=axis_color, tags="axes")
        self.canvas.create_text(X_OFFSET - 30, Y_OFFSET - 10, text="Tarefas", angle=90, anchor="w", fill=axis_color, font=("Segoe UI", 9), tags="axes")

    def _draw_gantt_bar(self, current_tick, task): #desenha o bloco da tarefa
        theme = "dark" if self.is_dark_theme.get() else "light"
        colors = THEMES[theme]
        if task.task_id not in self.task_y_positions:
            ...
            task_label = ttk.Label(legend_item_frame, text=f"- {task.task_id}", style="Legend.TLabel")
            self.style.configure("Legend.TLabel", background=colors["header_footer"], foreground=colors["text"])
        ...
        x1 = x0 + TICK_WIDTH
        self.canvas.create_rectangle(x0, y0, x1, y1, fill=task.color, outline=colors["background"])

    def _update_gui(self, task_that_ran): #atualiza a interface apos um passo do tick
        if not self.simulator: return
        current_tick = self.simulator.global_tick
        self.lbl_tick.config(text=f"Tick: {current_tick}")
        if task_that_ran: self._draw_gantt_bar(current_tick, task_that_ran)

    def _run_simulation_step(self): #executa um unico tick e atualiza a gui
        if not self.simulator: return False
    ...
    def _save_canvas_as_image(self): #captura a imagem salva
        try:
    ...
    def _next_step_clicked(self): #executa um passo por vez
        self.is_running = False
        self.btn_run_all.config(text="Executar Tudo")
        self._run_simulation_step()

    def _run_all_clicked(self): #chamada pelo botao executar tudo
        if self.is_running:
            self.is_running = False
            self.btn_run_all.config(text="Executar Tudo")
        else:
            if not self.simulator:
                messagebox.showwarning("Aviso", "Por favor, carregue um arquivo de simulação primeiro.")
                return
    ...
    def _run_loop(self):
      ...
      #loop principal do tkinter | esperando por eventos
    def start(self):
